...
export function CallPanel({ phone, setPhone, status, error, onCall, onHangup, customer, signals,
  objections, commitments, sensitive, preCallBrief }: any & { preCallBrief?: PreCallBrief | null }) {
  ...
  return <aside className="panel panel--profile"><h2>CALL &amp; PROFILE</h2><div className="dialer">
    ...
    <button className="button button--call" onClick={onCall} disabled={status === 'connected'}><PhoneIcon/>Call</button>
    <button className="button button--hangup" onClick={onHangup} disabled={status !== 'connected'}><HangupIcon/>Hang up</button></div>
